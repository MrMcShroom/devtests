...
Upload the files to your server and remove the `node_modules` folder, since the included copy might be outdated. This mostly matters if your server is reachable from the Internet - even one behind a password - so it's best to reinstall dependencies fresh rather than trust an old copy.

Then, in the command line (Linux, CentOS):
```
apt update
apt install nodejs
apt install npm
```
Navigate to the directory containing your Star Defenders 2D files (specifically, where `index.js` is), then run:
```
npm init
...
```
PS: You'll probably need the latest Node.js version. If something doesn't work, you can contact me or discuss it in #sd-discussion on the Discord server linked above.

PSS: It's all pretty much the same for Windows - just download Node.js from their official website (https://nodejs.org), then follow their instructions for running a simple Express application. Once that works, replace their example `index.js` with this project's files and run it. To debug using a Chromium browser, run it from the command line (cmd.exe) with:
```
